...
def test_admin_invite_code_api_requires_json_auth(app) -> None:
...
def test_failed_wallet_credit_keeps_settlement_in_recovery(app) -> None:
    ...
    invite = ReferralInviteCode(
        code="MISSING", profit_share_percent=10, percent_profit=10, profit_share_wallet="missingwallet", usage_count=1
    )
    db.session.add(invite)
    db.session.flush()
    invitee.referral_invite_code_id = invite.id
