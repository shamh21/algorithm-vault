...
def test_service_worker_registers_with_root_scope(app) -> None:
    ...
    assert "navigator.serviceWorker.register(window.AV_SW_URL" in app_shell
    assert 'scope: window.AV_SW_SCOPE || "/"' in app_shell
    assert 'updateViaCache: "none"' in app_shell
    assert 'worker.postMessage({ type: "SKIP_WAITING" })' in app_shell
...
def test_pwa_manifest_has_ios_install_shape(app) -> None:
    ...
    assert payload["orientation"] == "any"
    assert payload["background_color"] == "#050507"
    assert payload["theme_color"] == "#050507"
    ...
    assert {shortcut["short_name"] for shortcut in payload["shortcuts"]} >= {"Overview", "Wallet", "Vault"}
