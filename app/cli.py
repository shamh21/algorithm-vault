...
def register_cli(app: Flask) -> None:
    ...
    def activate_trading_connection(user_id: int, connection_id: int, confirm: str) -> None:
        """Activate one verified live trading connection and deactivate the user's others."""
        ...
        deactivated = [item for item in previous_payload if int(item["connection_id"]) != int(connection.id)]
        audit = AuditLog(
            category="connections",
            action="trading_connection_activated",
            message=f"{connection.provider.title()} trading connection activated by operator CLI.",
            user_id=user_id,
        ...
        audit.details = {
            ...
            "deactivated_connections": deactivated,
            "warning": "Activating one connection deactivates the user's other trading connections.",
        }
            ...
            json.dumps(
                {
                    ...
                    "provider": connection.provider,
                    "verification_status": connection.verification_status,
                    "deactivated_connection_ids": [item["connection_id"] for item in deactivated],
                    "warning": "Activating one connection deactivates the user's other trading connections.",
                },
                indent=2,
                default=str,
    ...
    @click.option("--connection-id", default=None, type=int)
    @click.option("--cap-usdc", default="10.0", show_default=True, help="USD notional cap, or all to use available collateral clipped by hard caps.")
    @click.option("--horizon", default="1h", show_default=True)
    @click.option(
        "--objective",
        default="risk_adjusted",
    ...
    micro_active_blockers = [
        ...
        *[
            ...
            in {
                ...
                "live_micro_canary_max_notional_invalid",
                ...
                "live_micro_canary_max_notional_exceeded",
...
def _first_canary_guardrail() -> dict[str, object]:
    preferred = max(0.0, _float(current_app.config.get("FIRST_CANARY_MAX_NOTIONAL_USDT"), 1.0))
    fallback = max(preferred, _float(current_app.config.get("FIRST_CANARY_FALLBACK_MAX_NOTIONAL_USDT"), 2.0))
    max_leverage = max(0.0, _float(current_app.config.get("FIRST_CANARY_MAX_LEVERAGE"), 1.0))
    return {
        "preferred_max_notional_usdt": preferred or 1.0,
        "fallback_max_notional_usdt": min(max(fallback, preferred or 1.0), 2.0),
        "max_leverage": max_leverage or 1.0,
        "fallback_policy": "Only use fallback when provider minimum order sizing blocks the preferred cap.",
...
def _live_micro_canary_guardrail() -> dict[str, object]:
    ...
    max_notional_usd = max(
        0.0,
        _float(current_app.config.get("LIVE_MICRO_CANARY_MAX_NOTIONAL_USD"), 10.0),
    )
    ...
    if enabled:
        ...
        if max_notional_usd <= 0:
            blockers.append("live_micro_canary_max_notional_invalid")
...
def _resolve_live_micro_canary_order_size(
    *,
    ...
    max_notional = _float(micro_guardrail.get("max_notional_usd"), 10.0)
    ...
    if leverage <= 0 or leverage > _float(micro_guardrail.get("max_leverage"), 1.0) + 1e-9:
        blockers.append("live_micro_canary_leverage_exceeds_one")
    if max_notional > 0 and final_notional > max_notional + 1e-9:
        blockers.append("live_micro_canary_max_notional_exceeded")
...
def _canary_effective_notional_cap(ranking: StrategyRanking, parameters: dict[str, object]) -> float:
    ...
    micro_guardrail = _live_micro_canary_guardrail()
    first_canary_cap = (
        _float(first_canary.get("fallback_max_notional_usdt"), 2.0)
        if (
        ...
        else _float(first_canary.get("preferred_max_notional_usdt"), 1.0)
    )
    caps = [
        first_canary_cap,
    ...
    *,
    ...
    context = {
        **dict(row or {}),
        ...
        "high_upside_profile": True,
        "optimizer_profile": "aggressive_1h",
        "allocation_amount_usd": current_app.config.get("HIGH_UPSIDE_MAX_POSITION_NOTIONAL_USD", 10.0),
        "hard_max_notional_usdc": current_app.config.get("HIGH_UPSIDE_MAX_POSITION_NOTIONAL_USD", 10.0),
...
def _ml_vault_exchange_leverage_gate(preview_payload: dict[str, object]) -> dict[str, object]:
    ...
    if _float(preview_payload.get("cap_usdc"), 0.0) > 10.0:
        blockers.append("notional_cap_above_10_usdc")
    return {
        "ready": not blockers,
        "policy": policy,
...
def _ml_live_vault_cap_readiness(bucket: str) -> dict[str, object]:
    blockers: list[str] = []
    max_notional = _float(current_app.config.get("HIGH_UPSIDE_MAX_POSITION_NOTIONAL_USD"), 0.0)
    ...
    if max_notional <= 0 or max_notional > 10.0:
        blockers.append("HIGH_UPSIDE_MAX_POSITION_NOTIONAL_USD_must_be_0_to_10")
...
def _ml_live_vault_dynamic_cap_payload(
    *,
    ...
    hard_caps = {
        ...
        "high_upside_max_position_notional_usd": max(
            0.0,
            _float(current_app.config.get("HIGH_UPSIDE_MAX_POSITION_NOTIONAL_USD"), 0.0),
        ),
    ...
    if _ml_objective(objective) == "extreme_upside":
        decision_context = {
            ...
            **context,
            ...
            "hard_max_notional_usdc": clipped_cap,
