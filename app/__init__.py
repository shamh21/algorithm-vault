...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    def inject_globals():
        mode = "live"
        user = current_user()
        is_admin_authenticated = admin_authenticated()
        return {
            "current_app": app,
            "nav_mode": mode,
            ...
            "seo": seo_context(
                ...
                path=request.path,
                authenticated=user is not None or is_admin_authenticated,
            ),
...
def _register_operational_routes(app: Flask) -> None:
    ...
    @app.get("/favicon.ico")
    def pwa_favicon():
        return redirect("/icons/favicon.ico", code=308)
...
def _block_when_database_startup_failed(app: Flask) -> Response | None:
    ...
    if (
        ...
        or path
        in {"/healthz", "/readyz", "/manifest.json", "/manifest.webmanifest", "/robots.txt", "/sitemap.xml", "/sw.js", "/favicon.ico"}
...
def _rate_limit_request(app: Flask) -> Response | None:
    ...
    if not bool(app.config.get("RATELIMIT_ENABLED", True)):
        return None
    if request.endpoint == "static" or request.path in {"/healthz", "/readyz", "/robots.txt", "/sitemap.xml"}:
        return None
...
def _set_response_headers(app: Flask, response: Response) -> Response:
    ...
    cacheable_static_response = (
        request.endpoint == "static"
        or path in {"/favicon.ico", "/manifest.json", "/manifest.webmanifest", "/sw.js"}
        or path.startswith("/icons/")
    )
    seo_asset_response = is_seo_asset_path(path)
    public_html_response = (
        is_public_indexable_path(path, endpoint=request.endpoint, authenticated=authenticated)
