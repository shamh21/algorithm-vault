(() => {
  ...
  class BacktestChart {
    ...
    applyData() { if (!this.chart || !this.lineSeries) return; const series = Array.isArray(this.payload?.charts?.[this.mode]) ? this.payload.charts[this.mode] : []; this.lineSeries.applyOptions({ color: this.mode === "pnl" ? "#9b4dff" : "#ff1f36" }); this.lineSeries.setData(series.map((row, index) => ({ time: this.timeValue(row.x, index), value: number(row.y) }))); this.chart.timeScale?.().fitContent?.(); }
    ...
    line(ctx, width, height) { const series = Array.isArray(this.payload?.charts?.[this.mode]) ? this.payload.charts[this.mode] : []; if (!series.length) return this.empty(ctx, width, height); const values = series.map((row) => number(row.y)); const min = Math.min(...values); const max = Math.max(...values); ctx.save(); ctx.strokeStyle = this.mode === "pnl" ? "rgba(155,77,255,0.92)" : "rgba(255,31,54,0.88)"; ctx.lineWidth = 2; ctx.beginPath(); series.forEach((row, index) => { const x = (index / Math.max(series.length - 1, 1)) * width; const y = height - ((number(row.y) - min) / Math.max(max - min, 1e-9)) * (height - 24) - 12; if (index === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }); ctx.stroke(); ctx.restore(); }
