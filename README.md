...
For Vercel-to-live-API vault routing, whitelist the fixed server/proxy egress IP in KuCoin, not the operator browser IP shown in KuCoin's API-key screen. If KuCoin runs directly from Vercel, configure either a KuCoin-only fixed-egress proxy with `KUCOIN_EGRESS_PROXY_URL` or `QUOTAGUARDSTATIC_URL`, or enable Vercel Static IPs and set `KUCOIN_NATIVE_STATIC_EGRESS_ENABLED=true`; otherwise leave KuCoin routed to a separate fixed-egress runtime. Set `KUCOIN_IP_RESTRICTION_MODE=trusted`, mirror the KuCoin trusted-IP list in `KUCOIN_TRUSTED_IPS`, and set the actual fixed egress IPs in `KUCOIN_EGRESS_PUBLIC_IPS`. The `/api/vault/kucoin-diagnostics` endpoint reports operator IP separately from server egress and never exposes KuCoin API secrets.
